...
RUN apt-get install -y hailo-all hailofw
...
set timeout 120\n\
spawn dpkg -i /app/hailo_assets/hailort-pcie-driver_4.23.0_all.deb\n\
expect "Do you wish to use DKMS?"\n\
send "Y\r"\n\
...
# Étape 33 SUPPRIMÉE : Ne compilez pas depuis les sources si vous installez les .deb
...
# ... (Continuez avec l'installation du .deb hailort_4.23.0_arm64.deb)
RUN dpkg -i /app/hailo_assets/hailort_4.23.0_arm64.deb || apt-get install -f -y

# CORRECTION IMPORT ERROR : Mettre à jour le cache ldconfig
RUN ldconfig
ENV LD_LIBRARY_PATH="/usr/lib/aarch64-linux-gnu/:$LD_LIBRARY_PATH" 

# Installation du wheel Python
RUN python -m pip install /app/hailo_assets/hailort-4.23.0-cp311-cp311-linux_aarch64.whl || true
